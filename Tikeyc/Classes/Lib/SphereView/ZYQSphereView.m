...
-(void)changeView{
    ...
    [self rotateSphereByAngle:self.speed fromPoint:normalPoint toPoint:movePoint];
}
...
- (void)setItems:(NSArray *)items {
	...
	[self rotateSphereByAngle:self.speed fromPoint:CGPointMake(0, 0) toPoint:CGPointMake(0, 1)];
}
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)panRecognizer {
	switch (panRecognizer.state) {
		...
		case UIGestureRecognizerStateChanged: {
			...
			// Sphere rotation
			[self rotateSphereByAngle:self.speed fromPoint:normalizedOriginalTouchPoint toPoint:normalizedTouchPoint];
		}
...
- (CGFloat)coordinateForNormalizedValue:(CGFloat)normalizedValue withinRangeOffset:(CGFloat)rangeOffset {
    ...
    //add by tikeyc 20170601 控制最小缩小倍数
    if (coordinate < self.miniScallValue) {
        coordinate = self.miniScallValue;
    }
	return coordinate;
}
