...
@property (strong, nonatomic) IBOutlet UIView *bottomToolsBgView;
...
- (JYJTopicVoiceView *)voiceView {
    ...
    _voiceView.currentIndexPath = self.currentIndexPath;
    return _voiceView;
}
...
- (JYJTopicVideoView *)videoView {
    if (!_videoView) {
        ...
        videoView.tableView = self.tableView;
        [self.contentView addSubview:videoView];
        self.videoView = videoView;
    }
    _videoView.currentIndexPath = self.currentIndexPath;
    return _videoView;
}
...
- (void)setTopic:(JYJTopic *)topic {
    _topic = topic;
    _moreButton.hidden = _isHome;
    _bottomToolsBgView.hidden = _isHome;
    // 新浪加V
