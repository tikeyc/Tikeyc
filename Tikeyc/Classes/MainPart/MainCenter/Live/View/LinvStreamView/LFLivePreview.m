...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.cameraButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(_closeButton.mas_left).offset(-20);
            make.top.equalTo(@40);
        }];
        
        [self.containerView addSubview:self.beautyButton];
        [self.beautyButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(_cameraButton.mas_left).offset(-20);
            make.top.equalTo(@40);
        }];
...
//        /***   默认分辨率368 ＊ 640  音频：44.1 iphone6以上48  双声道  方向竖屏 ***/
        ...
        videoConfiguration.videoSize = self.size;//CGSizeMake(640, 360);
