 ...
 */
+ (void)startBroadcastVoice:(id)user
                      speed:(NSInteger)speed
                     volume:(NSInteger)volume
                       tone:(NSInteger)tone
                      voice:(AVSpeechSynthesizer *)voice
...
+ (void)continueBroadcastVoice:(AVSpeechSynthesizer *)voice{
    
    [voice continueSpeaking];
}

+ (void)pauseBroadcastVoice:(AVSpeechSynthesizer *)voice{
...
+ (void)stopBroadcastVoice:(AVSpeechSynthesizer *)voice{
    ...
    [voice stopSpeakingAtBoundary:AVSpeechBoundaryWord];
}
...
@interface TWordsToVoiceViewController ()<UITextViewDelegate,AVSpeechSynthesizerDelegate>

@property (nonatomic, strong) AVSpeechSynthesizer *voice;
@property (nonatomic, copy) NSString *currentVoiceWords;
@property (nonatomic, copy) NSString *lastVoiceWords;

@property (strong, nonatomic) IBOutlet UITextView *textView;

@property (strong, nonatomic) IBOutlet UIButton *startButon;
- (IBAction)startButtonAction:(UIButton *)sender;

@end

@implementation TWordsToVoiceViewController
...
- (IBAction)startButtonAction:(UIButton *)sender {
    ...
    if (!_voice || ![self.textView.text isEqualToString:self.lastVoiceWords]) {
        
        if (_voice && _voice.isSpeaking) {
            //正在读的时候修改了文字，然后点击了暂停
        } else {
            self.lastVoiceWords = self.textView.text;
            sender.selected = NO;
            if (_voice) {
                [VoiceManager stopBroadcastVoice:_voice];
                _voice = nil;
            }
    ...
    if (sender.selected == NO) {
        ...
        if ([_voice isPaused]) {
            ...
            [VoiceManager continueBroadcastVoice:_voice];
            [sender setTitle:@"暂停" forState:UIControlStateNormal];
        }else{
            
            [VoiceManager startBroadcastVoice:self
                                        speed:0.5
                                       volume:1
                                         tone:1
                                        voice:_voice
        ...
        [VoiceManager pauseBroadcastVoice:_voice];//暂停
...
- (void)textViewDidEndEditing:(UITextView *)textView {
    self.currentVoiceWords = textView.text;
