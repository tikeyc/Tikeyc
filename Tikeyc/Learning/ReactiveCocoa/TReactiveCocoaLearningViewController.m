//
//  TReactiveCocoaLearningViewController.m
//  Tikeyc
//
...
@implementation TReactiveCocoaLearningViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    /*http://blog.csdn.net/u012847940/article/details/52250601
     *http://www.jianshu.com/p/e10e5ca413b7
     * ReactiveCocoa框架学习笔记 ReactiveCocoa框架学习笔记
     * ReactiveCocoa框架学习笔记 ReactiveCocoa框架学习笔记
     */
    
//    [self learningRACMulticastConnection];
}

#pragma mark - RACSiganl简单使用

- (void)learningRACSignal{
    // RACSignal使用步骤：
...
- (void)learningRACSubjectDelegate{
    /*
     // 需求:
...
#pragma mark - RACSequence和RACTuple简单使用 用于代替NSArray,NSDictionary,可以使用它来快速遍历数组和字典。

- (void)learningRACSequence{
    // 1.遍历数组
...
//    NSArray *dictArr = [NSArray arrayWithContentsOfFile:filePath];
    ...
    NSMutableArray *_flags;//此处为了方便查看，实际应定义在上边{}中或声明一个全局属性
    _flags = flags;
...
//    NSArray *dictArr = [NSArray arrayWithContentsOfFile:filePath];
    ...
    NSArray *flagss = [[dictArr.rac_sequence map:^id(id value) {
...
#pragma mark -  RACCommand简单使用 RAC中用于处理事件的类，可以把事件如何处理,事件中的数据如何传递，包装到这个类中，他可以很方便的监控事件的执行过程。

- (void)learningRACCommand{
    // 一、RACCommand使用步骤:
...
#pragma mark -  RACMulticastConnection:用于当一个信号，被多次订阅时，为了保证创建信号时，避免多次调用创建信号中的block，造成副作用，可以使用这个类处理。

- (void)learningRACMulticastConnection{
    ...
    RACSignal *signal2 = [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        ...
        return nil;
    }] timeout:10 onScheduler:[RACScheduler schedulerWithPriority:RACSchedulerPriorityBackground]];
...
#pragma mark -  以下是相对高级用法

#pragma mark - bind方法简单介绍和使用

- (void)learningRACbind{
...
#pragma mark - flattenMap，Map用于把源信号内容映射成新的内容
 ...
 */
- (void)learningRACFlattenMapAndMapMethod{
...
#pragma mark - ReactiveCocoa操作方法之组合。 concat:按一定顺序拼接信号，当多个信号发出的时候，有顺序的接收信号 then:用于连接两个信号，当第一个信号完成，才会连接then返回的信号。

- (void)learningRAC_concatAnd_thenMethod{
    ...
    RACSignal *signalB2 = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
    ...
    RACSignal *mergeSignal = [signalA2 merge:signalB2];
    ...
    RACSignal *signalA5 = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
    ...
    RACSignal *signalB5 = [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
    ...
    RACSignal *reduceSignal = [RACSignal combineLatest:@[signalA5,signalB5] reduce:^id(NSNumber *num1 ,NSNumber *num2){
